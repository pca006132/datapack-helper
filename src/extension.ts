...
import {evaluate, removeDuplicated} from './script-runner';
...
export function activate(context: vscode.ExtensionContext) {
	...
	vscode.commands.registerTextEditorCommand("datapack.removeduplicated", (editor, edit) => {
		if(!enabled)
			return;
		editor.edit((editBuilder)=> {
			for(const v of editor.selections) {
				editBuilder.replace(v, removeDuplicated(editor.document.getText(v)));
			}
		}).then((value)=> {
			if (!value) vscode.window.showErrorMessage("replace failed");
		})
